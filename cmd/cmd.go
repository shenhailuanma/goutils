...
type Options struct {
	...
	// If Interlacing is true, STDOUT and STDERR are written to same buffer or channel
	// that like normal terminal output.
	Interlacing bool
}
...
func NewCmdOptions(options Options, name string, args ...string) *Cmd {
	...
	if options.Buffered {
		if options.Interlacing {
			c.stdoutBuf = NewOutputBuffer()
			c.stderrBuf = c.stdoutBuf
		} else {
			c.stdoutBuf = NewOutputBuffer()
...
func (c *Cmd) Status() Status {
	...
	if c.done {
		...
		if !c.final {
			if c.stdoutBuf != nil {
				c.status.Stdout = c.stdoutBuf.String()
				c.status.Stderr = c.stderrBuf.String()
...
func (rw *OutputBuffer) String() string {
